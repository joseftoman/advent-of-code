...
my $min = knapsack($sum / 4, \@weights);
my $best;

do {
  $best = get_best($min, $sum / 4, \@weights);
  $min++;
...
sub get_best {
  ...
  while (my $set = $iter->next()) {
    ...
    next unless has_3_subsets($goal, \@diff);
...
sub has_3_subsets {
  ...
  for my $size (1..int(scalar(@$items) / 3)) {
    my $iter = Algorithm::Combinatorics::combinations($items, $size);
    while (my $set = $iter->next()) {
      my $score = 0;
      $score += $_ for @$set;
      ...
      my %map = map { $_ => 1 } @$set;
      my @diff = grep { not exists $map{$_} } @$items;
      return 1 if defined knapsack($goal, \@diff);
    }
  }
